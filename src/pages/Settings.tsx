...
export function Settings({ user, onNavigate }: SettingsProps) {
    ...
    useEffect(() => {
        try {
            const saved = sessionStorage.getItem('moduly_settings_back_to');
    ...
    const [appearance, setAppearance] = useState<AppearanceTheme>(() => {
        try {
            const saved = localStorage.getItem('moduly_theme');
            if (saved) return saved as AppearanceTheme;
        } catch (err) {}
        return (document.documentElement.getAttribute('data-theme') as AppearanceTheme) || 'system';
    });
    ...
    useEffect(() => {
        ...
        try {
            localStorage.setItem('moduly_theme', appearance);
        } catch (err) {}
    }, [appearance]);
                ...
                <div className="settings-container">
                    <div className="settings-breadcrumb">
                        ...
                        >
                            ...
                            <span>Back to {backTo === 'admin' ? 'Admin Panel' : 'Dashboard'}</span>
                        </button>
                    </div>
